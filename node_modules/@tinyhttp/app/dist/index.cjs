...
const extendMiddleware = (app) => (req$1, res$1, next) => {
    ...
    Object.defineProperty(req$1, 'fresh', { get: req.getFreshOrStale.bind(null, req$1, res$1), configurable: true });
...
class App extends router.Router {
    ...
    use(...args) {
        ...
        for (const fn of fns) {
            if (fn instanceof App) {
                regex = regexparam.parse(path, true);
                fn.mountpath = path;
                this.apps[path] = fn;
                fn.parent = this;
            }
    ...
    find(url) {
        return this.middleware.filter((m) => {
            m.regex = m.regex || regexparam.parse(m.path, m.type === 'mw');
            let fullPathRegex;
            m.fullPath && typeof m.fullPath === 'string'
                ? (fullPathRegex = regexparam.parse(m.fullPath, m.type === 'mw'))
                : (fullPathRegex = null);
    ...
    handler(req$1, res, next) {
        ...
        const mw = [
            ...
            ...matched.filter((x) => req$1.method === 'HEAD' || (x.method ? x.method === req$1.method : true))
        ];
        if (matched[0] != null) {
            mw.push({
                type: 'mw',
        ...
        const handle = (mw) => async (req$1, res, next) => {
            ...
            if (path.includes(':')) {
                ...
                const url = req$1.url.slice(req$1.url.indexOf(first) + (first === null || first === void 0 ? void 0 : first.length));
                req$1.url = lead(url);
            }
            else {
